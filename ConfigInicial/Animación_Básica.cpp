/*
*	Luis Arturo Meza Sánchez
*	Practica 9: Animacion Basica
*	Fecha de entrega: 18 de Oct de 2024
*/
...
bool active;
// para la practica, varaibles para verificar el limite de la pelotita
bool arriba = false, abajo = true;
...
float cambiopos = 0; // Para la practica para la animación de que sube y baja

// --------------------- Para la practica ---------------------------
// Para que este en lados opuestos
float iniDog = 0.0f;
float iniBall = 3.14f;
...
float posperroX = -1.0f;
float posperroZ = -1.0f;
float yDog = 0.0f;
float rotDog = iniDog;
bool AnimDog = false;
float angulo = 0.0f;
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion basica | Luis Arturo Meza Sanchez", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(posperroX, yDog, posperroZ)); //Translada al perro en la trayectoria circular 
		//se aplica la rotaci�n con la variable que ser� afectada por la animacion 
		model = glm::rotate(model, angulo, glm::vec3(0.0f, 1.0f, 0.0f)); // Rotaci�n del perro seg�n su posici�n en la circunferencia
		...
		model = glm::translate(model, glm::vec3(posbolaX, yBall, posbolaZ)); // Mover la pelota seg�n la posici�n 
		//se manda al shader para verse reflejado en el modelo 
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void Animation() {
	...
	{
		//rotBall += 0.2f;
		// Animación para subir que empieza con abajo = TRUE
		if (cambiopos >= 0.0 and abajo == true) { // Pongo 1.7 porque con 2.0 se pasa
			cambiopos += 0.01;
			if (cambiopos >= 1.7) {
				abajo = false;
				arriba = true;
		...
		if (cambiopos <= 2.0 and arriba == true) { // Pongo 1.7 porque con 2.0 se pasa
			cambiopos -= 0.01;
			if (cambiopos <= 0.0) {
				cambiopos = 0.0;
				abajo = true;
	...
	// Para el circulo de la pelita
	posbolaZ = 2.0f * sin(rotBall);
	posbolaX = 2.0f * cos(rotBall);


	// Para el circulo del perro 
	if (AnimDog)
	{
		rotDog += 0.001f;
		if (rotDog > 2 * 3.14f) {
	...
	posperroZ = 2.0f * sin(rotDog);
	posperroX = 2.0f * cos(rotDog);
	angulo = atan2(-sin(rotDog), cos(rotDog));

	// Para el salto del perro

	if (AnimDog) {
		if (abs(posperroX - posbolaX) < 0.1f && abs(posperroZ - posbolaZ) < 0.1f && !salto) {
			salto = true;
			subiendo = true;
		}
	}
