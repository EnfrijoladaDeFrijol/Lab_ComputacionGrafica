/*
	...
	MEZA SÁNCHEZ lUIS ATRTURO
	FECHA DE ENTREGA: 03/NOVIEMBRE/2023
*/
...
// Variable para hacer los keyframes
...
// ------------ Funciones para guardar los datos en el archivo ----------------------
void guardarAnimacion(const char* filename = "Animaciones/animP11.txt") {
	...
	for (int i = 0; i < FrameIndex; i++) {
		file << KeyFrame[i].dogPosX << " " << KeyFrame[i].dogPosY << " " << KeyFrame[i].dogPosZ << " " << KeyFrame[i].rotDog 
			<< " " << KeyFrame[i].head << " "<< KeyFrame[i].tail << " " << KeyFrame[i].FLegs << " " << KeyFrame[i].FLegsL << " "
			<< KeyFrame[i].FLegsR << " " << KeyFrame[i].RLegs << " " << KeyFrame[i].rotDogX << "\n";
	}
	file.close();
	std::cout << "La animacion se escribio en el archivo" << std::endl;
}


void obtenerAnimation(const char* filename = "Animaciones/animP11.txt") {
	std::ifstream file(filename);
	if (!file.is_open()) {
		std::cerr << "Error al abrir el archiv" << std::endl;
		return;
	}
	FrameIndex = 0;
	while (FrameIndex < MAX_FRAMES &&
		file >> KeyFrame[FrameIndex].dogPosX >> KeyFrame[FrameIndex].dogPosY >> KeyFrame[FrameIndex].dogPosZ >> KeyFrame[FrameIndex].rotDog 
		>> KeyFrame[FrameIndex].head >> KeyFrame[FrameIndex].tail >> KeyFrame[FrameIndex].FLegs >> KeyFrame[FrameIndex].FLegsL
		>> KeyFrame[FrameIndex].FLegsR >> KeyFrame[FrameIndex].RLegs >> KeyFrame[FrameIndex].rotDogX) {
		FrameIndex++;
	}
}
// ------------------------------------------------------------------------
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion 3 | Arturo Meza Sanchez", nullptr, nullptr);
...
{
	if (key == GLFW_KEY_P) {
		guardarAnimacion();  // Guarda la animación en "Animacion.txt"
	}
